...
狂风冲锋加成
洪水面积太大
中毒 等异常 回合开始生效而不是结束生效

偃月阵
推 拉(禁军级别) 一个或者多个 4个等级 只对步兵有效
马：
1. 降低冲锋成功率，追击杀伤率, 容易被佯攻
2. 一般
3. 增加追击杀伤率 + 提高冲锋成功率
4. 冲锋成功率 + 破阵几率

步：
1. 容易被佯攻，容易被冲阵, 佯攻容易失败
2. 一般
3. 增加佯攻成功率, 不易被冲阵
4. 佯攻大量成功，不易被冲阵
...
性格：冲动(被佯攻, 不易被冲阵) 勇猛(冲阵，容易战死, 绝地反击--人少时) 沉稳(不易被佯攻) 忠义(总是参加协同) 功利（不参加协同） 贪生怕死(容易被shake吓退, 不参加协同)
